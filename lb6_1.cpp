...
int main(int argc, char* argv[]) 
{
	...
	if (rank == 0) {
        ...
        rows += 1;
		columns = count / rows; 
        //''' comment in for example run on 3x3'''
        columns = 30;
        rows = 30;
        std::cout <<  columns << " = " << count << " / " << rows << std:: endl;
    ...
    if (rank == 0) 
	{
        ...
        std::string file_name = "result_no_cache_dot.txt";
        out_file.open(file_name, std::ios::app);
        out_file << worldSize << "\t" << (endtime - starttime) * 1000000 << std::endl;
        out_file.close();
    }
