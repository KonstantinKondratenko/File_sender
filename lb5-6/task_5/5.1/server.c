...
int main(int argc, char** argv) 
{
    ...
    if(semop(semaphore, setWriteEnable, 1) < 0) {
    ...
    for(;;) 
    {
        ...
        /*
            Функция semop() атомарно выполняет операции с семафорами над набором семафоров, связанных с аргументом semid.
            Аргумент sops — это указатель на массив структур данных sembuf.
            Аргумент nsops — это количество структур sembuf в массиве.
        */
        // ждем пока клиент начнет работу
        if(semop(semaphore, readEnable, 1) < 0) 
        {
            printf("execution complete\n");
            kill(getpid(),SIGINT);
        }
